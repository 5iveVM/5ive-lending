...
3. **Use `.ctx.key` on account-typed values** to extract public keys for comparisons and assignments.
...
4. `get_key(account_like) -> pubkey` (widely used in templates/examples; `param.ctx.key` remains preferred in most contract code)
